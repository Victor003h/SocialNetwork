...
Node 1:
```bash
docker run -d   --name node1  --network cluster_net --network-alias cluster_net_serv  -e  NODE_ID=1   -e NODE_PORT=5000 -e POSTGRES_HOST=db1-postgres   -e SERVICE_NAME=cl_service   db_cluster_node

```

Node 2:
```bash
docker run -d   --name node1  --network cluster_net --network-alias cluster_net_serv  -e  NODE_ID=1   -e NODE_PORT=5000 -e POSTGRES_HOST=db2-postgres   -e SERVICE_NAME=cl_service   db_cluster_node
